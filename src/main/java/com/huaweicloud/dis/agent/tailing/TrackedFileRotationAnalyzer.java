...
/**
 * A class encapsulating various heuristics and methods to assist in tracking tailed files, to be used by
 * {@link SourceFileTracker}. A few definitions:
 * <ul>
 * <li>current snapshot: the last snapshot of files processed by {@link SourceFileTracker#refresh()}.</li>
 ...
 * <li>file counterpart: the incoming (current) file with the same {@code FileId} as a current (incoming) file
 * (respectively).</li>
 * <li>matched file (M): a file whose counterpart was found.</li>
 * <li>un-matched file (X): a file (current or incoming) whose counterpart was not found.</li>
 * </ul>
 */
@NotThreadSafe
public class TrackedFileRotationAnalyzer
{
    private static final Logger LOGGER = LoggerFactory.getLogger(TrackedFileRotationAnalyzer.class);
    ...
    private int noCounterpartsCount;
    ...
    public TrackedFileRotationAnalyzer(TrackedFileList current, TrackedFileList incoming, TrackedFile currentOpenFile)
    {
        ...
        Preconditions.checkArgument(currentOpenFile == null || current.contains(currentOpenFile),
            "Current file is not contained in current snapshot!");
        Preconditions.checkArgument(incoming != current, "Current and incoming are the same list!"); // identitiy
                                                                                                     // intended
    ...
    {
        this(current, incoming, currentOpenFile);
     ...
     * <li>Only tail files in current snapshot are allowed not to have counterparts: older files deleted by
     * rotation</li>
     ...
     * </ol>
     *
     * @return {@code true} if no rotation appears to be have happeneded, otherwise {@code false}.
     */
    public boolean checkNoRotation()
    {
        for (TrackedFile incomingFile : incoming)
        {
            ...
            if (!incomingFile.isSameAs(currentCounterpart)
                ...
                return false;
            if (currentCounterpart == currentOpenFile)
            {
                // identity intended
    ...
    /**
     * After rotation by Truncate (assuming no anomalies), we might end up with any of following in the incoming
     * snapshot (X=un-matched, M=matched):
     * <ol>
     ...
     * </ol>
     * In cases 1 and 2 (no prior rotations), we assume that the current file is now the oldest unmatched file.
     * <p>
     * In cases 3 and 4 (with one or more prior rotations):
     * <ul>
     * <li>If current file is matched to any of the prior rotated files (Mi), then the current remains the same</li>
     * <li>Otherwise, the oldest un-matched file is assumed to be the current file</li>
     * </ul>
     * TODO: For the cases where the oldest un-matched (Xn) is assumed to be the current file, a comparision of content
     * hash can be useful to confirm this.
     *
     * @return The best guess of the index of the current file in the incoming list, or {@code -1} if no such guess was
     *         possible.
     */
                ...
                if (hasCounterpart(f))
                {
                    ...
                    if (foundFirstMatch)
                    {
                        ...
                        if (counterpart.getLastModifiedTime() != f.getLastModifiedTime())
                            otherAnomalies.add("last modified time changed from " + counterpart.getLastModifiedTime()
                                + " to " + f.getLastModifiedTime());
                        if (previousCounterpartIndex >= 0)
                        {
                            if (counterpartIndex < previousCounterpartIndex)
                                otherAnomalies.add("file relative position changed (expected to be after position "
                                    + previousCounterpartIndex + ")");
                        }
                    }
                    else
                    {
    ...
    /**
     * Checks if the current open file has been truncated, according to a heuristic (see implementation). TODO: The
     * heuristic has a weakness: even if file has same/increased size, it could still be truncated and then popualated
     * by the same/more amount of bytes. To make this more robust and handle such edge cases we could do a content
     * comparision by hash.
     *
     * @return {@code true} if the current open file has likely been truncated, otherwise {@code false}.
     */
    public boolean currentOpenFileWasTruncated()
    {
        TrackedFile counterpart = getCounterpart(currentOpenFile);
        return counterpart != null && counterpart.getPath().equals(currentOpenFile.getPath())
    ...
    public boolean trackedFileWasTruncated(TrackedFile trackedFile)
    {
        TrackedFile counterpart = getCounterpart(trackedFile);
        return counterpart == null
            || (counterpart.getPath().equals(trackedFile.getPath()) && counterpart.isSameAs(trackedFile)
                && counterpart.getLastModifiedTime() >= trackedFile.getLastModifiedTime()
                && counterpart.getSize() < trackedFile.getSize());
    }
    
    /**
     * 如果incoming列表为空 或者 所有current文件在incoming中没有相同的文件，则认为所有文件都清理了。
     *
     * @return {@code true} if all currently tracked files have disappeared, {@code false} otherwise.
     */
    public boolean allFilesHaveDisappeared()
    {
        return !current.isEmpty() && (incoming.isEmpty() || !hasMatchedFilesAtOrAfter(current, 0));
    }
    
    /**
     * @param f
     * @return {@code true} if {@code getCounterpart(f)} would return non-{@code null}, {@code false} otherwise.
     */
    ...
    /**
     * The "counterpart" of file {@code f}, defined as the file with the same {@code FileId} in the reciprocal snapshot
     * (if {@code f} is in {@code current}, then the counterpart is in {@code incoming} and vice-versa).
     *
     * @param f
     * @return
     */
    ...
    /**
     * @param f
     * @return The index of the counterpart (see {@link #getCounterpart(TrackedFile)}) if in the corresponding snapshot
     *         ({@code current} or {@code incoming}), or {@code -1} if {@code f} has no counterpart.
     */
    ...
    /**
     * Compares the attributes of {@code incomingFile} (I) to those of {@code currentOpenFile} (C) and determines
     * whether I could have been C a while a ago. This heuristic assumes that I could have been C if all the following
     * are true:
     * <ol>
     ...
     * </ol>
     * The logic being that files are being appended to, and therefore their size is going to increase and their
     * timestamp going to be updated with every write. If we find that the size if I decreased or its timestamp rolled
     * back (relative to C), then I could not have been C.
     *
     * @param incomingFile
     * @return {@code true} if the {@code incomingFile} could have been the same as {@code currentFile} at a previous
    ...
    /**
     * Matches all files in {@code newSnapshot} (except the top two) to the {@code currentSnapshot}. {@code FileId} is
    ...
    public int getIncomingNoCounterpartsCount()
    {
        return this.noCounterpartsCount;
    }
    
    public List<TrackedFile> getCurrentNoCounterparts()
    {
        List<TrackedFile> currentNoCounterpartsCount = new ArrayList<>();
        for (TrackedFile trackedFile : current)
        {
            if (counterparts.get(trackedFile) == null)
            {
                currentNoCounterpartsCount.add(trackedFile);
            }
        }
        return currentNoCounterpartsCount;
    }
}
