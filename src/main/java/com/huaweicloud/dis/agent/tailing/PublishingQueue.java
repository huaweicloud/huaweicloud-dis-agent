...
/**
 * A queue that keeps track of record buffers (aka batches) ready for publishing. It tracks buffers that were never
 * published (the "never-published queue") as well as buffers that were previously published unsuccessfully and are
 * slated for a retry (the "retry queue").
 * <p>
 * Buffers in the "retry queue" are always given priority over those in the "never-published queue" when polling.
 * Specifically, {@link #take()} returns any buffers in the retry-queue immediately (or as they become available).
 * <p>
 * New records (added via {@link #offerRecord(IRecord)}) are kept in a temporary buffer until it "matures" (based on
 * implementation-specific policy and/or configuration, such as maximum time or maximum age), at which time it's pushed
 * into the bounded "never-published queue".
 * <p>
 * A buffer "matures" (and is queued for processing) based on any one of the following three criteria:
 * <ul>
 ...
 * <li>Age of the buffer (time since first record was added): this class will try to maintain it below the value set by
 * {@link FileFlow#getMaxBufferAgeMillis()}; however this is only checked before/after buffers are removed from the
 * queue (in calls to {@link #take()} and {@link #tryTake()}) or with an explicit call to {@link #checkPendingRecords()}
 * because for a consumer the behavior is the same. If {@link #offerRecord(IRecord) offerRecord} was called at least
 * once and no calls to {@link #take()}/{@link #tryTake()}/ {@link #checkPendingRecords()} followed, the buffer age can
 * grow indefinitely.</li>
 * </ul>
 *
...
public final class PublishingQueue<R extends IRecord> implements IHeartbeatProvider
{
    ...
    public PublishingQueue(FileFlow<R> flow, int capacity)
    {
        ...
        try
        {
            ...
            if (checkPendingRecordsBeforeNewRecord(record, block))
            {
                if (LOGGER.isDebugEnabled())
                {
                    ...
                    Preconditions.checkState(currentBuffer.sizeBytesWithOverhead() + record.lengthWithOverhead() <= flow
    ...
    /**
     * @return {@code true} either if the temp buffer does not need to be queued or if was queued successfully, and
     *         {@code false} if it needed to be queued, but could not for any reason.
     */
    public boolean checkPendingRecords()
    {
        ...
        try
        {
            if (!currentBuffer.isEmpty() && (currentBuffer.sizeBytesWithOverhead() >= flow.getMaxBufferSizeBytes()
    ...
    public int discardPendingRecords()
    {
        ...
        try
        {
            ...
            if (!currentBuffer.isEmpty())
            {
                LOGGER.trace("{}: Discarding {} records in the temporary buffer...", name, currentBuffer.sizeRecords());
            }
            currentBuffer = new RecordBuffer<>(flow);
            return discarded;
        }
    ...
    private boolean checkPendingRecordsBeforeNewRecord(R record, boolean block)
    {
        ...
        try
        {
            if (!currentBuffer.isEmpty() && (currentBuffer.sizeBytesWithOverhead() + flow.getPerBufferOverheadBytes()
                + record.lengthWithOverhead() > flow.getMaxBufferSizeBytes()
                || currentBuffer.sizeRecords() >= flow.getMaxBufferSizeRecords()))
            {
    ...
    public String toString()
    {
        ...
        sb.append(getClass().getSimpleName())
            .append("(neverPubQueueSize=")
            .append(neverPubQueue.size())
            // .append(",neverPubQueueCapacity=").append(neverPubCapacity)
            .append(",retryQueueSize=")
            .append(retryQueue.size())
            .append(",totalRecords=")
            .append(totalRecords())
            // .append(",totalBytes=").append(totalBytes())
            .append(",pendingRecords=")
            .append(pendingRecords())
        ...
        {
            {
                ...
                put("PublishingQueue.AverageQueueWaitTimeMillis",
                    totalQueuedBuffers.get() == 0 ? 0.0
                        : (totalQueueWaitTimeMillis.doubleValue() / totalQueuedBuffers.doubleValue()));
    ...
    public boolean queueCurrentBuffer(R record, TrackedFile currentFile)
    {
        ...
        try
        {
            ...
            // Add record
            if (record != null && !record.shouldSkip())
            {
                while (isOpen && !(success = checkPendingRecordsBeforeNewRecord(record, true)))
                {
                    LOGGER.warn("Failed to checkPendingRecordsBeforeNewRecord... retry.");
                    try
                    {
                        Thread.sleep(1);
                    }
            ...
            {
                // 设置最新的文件信息
                currentBuffer.setTrackedFile(currentFile);
            }
            while (isOpen && !(success = queueCurrentBuffer(true)))
            {
                try
                {
                    Thread.sleep(1);
