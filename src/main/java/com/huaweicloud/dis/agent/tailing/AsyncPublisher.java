...
/**
 * A publisher that buffers records into an {@link PublishingQueue}, and can make send requests asynchronously.
 *
 * @param <R> The record type.
 */
class AsyncPublisher<R extends IRecord> extends SimplePublisher<R>
{
    private static final Logger LOGGER = LoggerFactory.getLogger(AsyncPublisher.class);
        ...
        ISender<R> sender, ExecutorService sendingExecutor)
    {
        ...
        this.throttler =
            new AsyncPublisherThrottler<>(this, flow.getRetryInitialBackoffMillis(), flow.getRetryMaxBackoffMillis());
    }
    
    /**
     * @return {@code false} if there are any records queued for sending or currently being sent (e.g. asynchronously),
     *         else {@code true}.
     */
     ...
     * @param unit
     * @return {@code true} if idle state was reached before the timeout expired, or {@code false} if idle state was not
    ...
    public boolean waitForIdle(long timeout, TimeUnit unit)
    {
        ...
        {
            long remaining =
                timeout > 0 ? (unit.toMillis(timeout) - timer.elapsed(TimeUnit.MILLISECONDS)) : Long.MAX_VALUE;
            if (remaining <= 0)
                return false;
    ...
    protected synchronized boolean onSendPartialSuccess(RecordBuffer<R> buffer, BufferSendResult<R> result)
    {
        ...
        try
        {
            ...
            {
                throttler.backoff();
                return super.onSendPartialSuccessAndRetry(buffer, result);
            }
            else
            {
    ...
    public String toString()
    {
        ...
        sb.append(getClass().getSimpleName())
            ...
            .append("queue=")
            .append(queue)
            .append(",activeSendTasks=")
            .append(activeSendTasks.get())
            .append(")");
        return sb.toString();
    }
