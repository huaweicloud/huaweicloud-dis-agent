...
/**
 * Class to handle reading configurations. A configuration is essentially a <code>Map&lt;String, Object&gt;</code>
 * that's interpreted by a configurator.
 * <p>
 * This class construnct a configuration map from a JSON stream (read from a file, e.g.): <code><pre>
...
public class Configuration
{
     ...
     */
    public static Configuration get(Path file)
        throws IOException
    {
    ...
    public static Configuration get(String configFile)
    {
        if (configFile.endsWith(Constants.YAML_CONFIG))
        {
            return new Configuration(new YamlConfig().parse(configFile));
        }
        else
        {
            return new Configuration(new JsonConfig().parse(configFile));
        }
    }
    ...
    public static Configuration get(InputStream is)
        throws IOException
    {
        ObjectMapper mapper = new ObjectMapper();
        mapper.configure(Feature.ALLOW_COMMENTS, true);
        return new Configuration((Map<String, Object>)mapper.readValue(is, new TypeReference<HashMap<String, Object>>()
        {
        }));
    }
    ...
    /**
     * @return an unmodifiable copy of the map that's backing this reader instance.
     */
    ...
    /**
     * @param key
     * @return <code>true</code> if configuration map contains the given key, <code>false</code> otherwise.
     */
    ...
    /**
     * Reads a required value of type <code>T</code> from the configuration map, applying conversion as needed. If the
     * key is not found in the configuraiton, a {@link ConfigurationException} is raised.
     *
     * @param key
     * @param clazz
     * @return
     * @throws ConfigurationException if this class does not know how to read and convert values of the specified type,
     *             or if the provided key does not exist in the configuration.
     */
    ...
    /**
     * Reads an optional value of type <code>T</code> from the configuration map, applying conversion as needed.
     *
     * @param key
     * @param clazz
     * @param fallback
     * @return the config value at given key, or <code>fallback</code> if the key does not exist in the configuration.
     *         Note that if the key exists its value is returned, even if it's <code>null</code>.
     * @throws ConfigurationException if this class does not know how to read and convert values of the specified type.
     */
     ...
     * @param key
     * @param fallback
     * @return the enum constant at given key, or <code>fallback</code> if the key does not exist in the configuration.
     * @throws ConfigurationException if the enum type is <code>null</code> or the specified enum type has no constant
     *             with the specified value read from key
     */
    public <E extends Enum<E>> E readEnum(Class<E> enumType, String key, E fallback)
    {
        ...
        if (stringVal != null)
        {
            ...
            {
                throw new ConfigurationException(String.format("Value(%s) is not legally accepted by key: %s. "
                    + "Legal values are %s", stringVal, key, Joiner.on(",").join(enumType.getEnumConstants())), e);
            }
        }
        else
            return fallback;
    ...
    public <E extends Enum<E>> E readEnum(Class<E> enumType, String key)
    {
        ...
        if (stringVal != null)
        {
            ...
            throw new ConfigurationException("Required configuration value missing: " + key);
    }
     ...
     * @return
     */
    public <T> List<T> readList(String key, Class<T> itemType, Function<Object, T> itemConverter)
    {
    ...
    /**
     * Helper class to parse configuration values and return expected types after necessary conversion.
     *
            ...
            else
            {
                ...
                catch (Exception e)
                {
                    throw new ConfigurationException(
                        String.format("Failed to convert value (%s) to desired type (%s): %s",
                            value.toString(),
                            this.clazz.toString(),
                            key),
                        e);
    ...
    /**
     * Reader for lists in configuration. It will apply the converter for each scalar value in the list.
     *
    ...
    class ListReader<T> implements ValueReader<List<T>>
    {
        ...
        public List<T> read(String key)
        {
            ...
            if (configMap.containsKey(key))
            {
                ...
                {
                    List<?> inputList = value.getClass().isArray() ? Arrays.asList(value) : (List<?>)(value);
                    ...
                    throw new ConfigurationException("Configuration value is not a list: " + value.toString());
            }
            else
                throw new ConfigurationException("Required list missing: " + key);
        }
        ...
        private T convertItem(String key, Object item)
        {
            ...
            else
            {
                ...
                {
                    throw new ConfigurationException(
                        String.format("Failed to convert list item (%s) to desired type (%s): %s",
                            item.toString(),
                            this.itemClazz.toString(),
                            key),
                        e);
                }
            }
    ...
    public static <T extends Comparable<T>> void validateRange(T value, Range<T> range, String label)
    {
        if (!range.contains(value))
        {
            String message = String
                .format("'%s' value %s is outside of valid range (%s)", label, value.toString(), range.toString());
